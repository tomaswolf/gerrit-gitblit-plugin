...
public class GitBlitDiffFormatter extends DiffFormatter {
	...
	/**
	 * Global diff limit. Commitdiffs with more lines are truncated. Can be reduced (but not increased) through gitblit.properties key
	...
	/**
	 * Position in the stream when we try to write the first line. Used to rewind when we detect that the diff is too large.
	 */
