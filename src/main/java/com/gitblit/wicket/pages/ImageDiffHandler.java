...
public class ImageDiffHandler implements DiffUtils.BinaryDiffHandler {
	...
	public String renderBinaryDiff(DiffEntry diffEntry) {
		...
		case RENAME:
		case COPY:
			// TODO: for very small images such as icons, the slider doesn't really help. Two possible approaches: either upscale them for display
			// (may show blurry upscaled images), or show them side by side (may still be too small to really make out the differences).
	...
	/**
	 * Constructs a URL that will fetch the designated resource in the git repository. The returned string will contain the URL fully URL-escaped, but note that
	 * it may still contain unescaped ampersands, so the result must still be run through HTML escaping if it is to be used in HTML.
	 * 
	...
	/**
	 * Encode a URL component of a {@link RawServlet} URL in the special way that the servlet expects it. Note that the %-encoding used does not encode '&amp;'
	 * or '&lt;'. Slashes are not encoded in the result.
	 * 
	 * @param component
	...
	protected String urlencode(final String component) {
		// RawServlet handles slashes itself. Note that only the PATH_INSTANCE fits the bill here: it encodes spaces as %20, and we just
		// have to correct for encoded slashes. Java's standard URLEncoder would encode spaces as '+', and I don't know what effects that
		// would have on other parts of GitBlit. It would also be wrong for path components (but fine for a query part), so we'd have to
		// correct it, too.
		//
		// Actually, this should be done in RawServlet.asLink(). As it is now, this may be incorrect if that operation ever uses query
		// parameters instead of paths, or if it is fixed to urlencode its path components. But I don't want to touch that static method
		// in RawServlet.
