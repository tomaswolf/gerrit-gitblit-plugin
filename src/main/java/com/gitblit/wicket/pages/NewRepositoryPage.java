...
public class NewRepositoryPage extends RootSubPage {
	...
	protected void onInitialize() {
		...
		AccessRestrictionType defaultRestriction = AccessRestrictionType.fromName(app().settings().getString(Keys.git.defaultAccessRestriction,
		...
		AuthorizationControl defaultControl = AuthorizationControl.fromName(app().settings().getString(Keys.git.defaultAuthorizationControl,
		...
		form.add(new BooleanChoiceOption<String>("addGitIgnore", getString("gb.initWithGitignore"), getString("gb.initWithGitignoreDescription"),
				addGitignoreModel, gitignoreModel, gitignores).setVisible(gitignores.size() > 0));

		// TODO consider gitflow at creation (ticket-55)
		addGitflowModel = Model.of(false);
		form.add(new BooleanOption("addGitFlow", "Include a .gitflow file",
				"This will generate a config file which guides Git clients in setting up Gitflow branches.", addGitflowModel).setVisible(false));
	...
	protected boolean initialCommit(RepositoryModel repository, boolean addReadme, String gitignore, boolean addGitFlow) {
		...
		try (Repository db = app().repositories().getRepository(repositoryModel.name); ObjectInserter odi = db.newObjectInserter()) {
